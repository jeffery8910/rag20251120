...
import { upsertChunks, deleteBySource } from '@/lib/pinecone';
...
export async function POST(req: NextRequest) {
  ...
  if (backend === 'pinecone') {
    ...
    return new Response('Vector store not configured (set Pinecone or Atlas envs)', { status: 500 });
  }
  return new Response(JSON.stringify({ inserted: docs.length }), {
